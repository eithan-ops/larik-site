...
function speak(t: string) {
  try {
    ...
    const v = ss.getVoices().find((x) => x.lang.toLowerCase().startsWith("he"));
    if (v) u.voice = v;
...
export default function SpodsView({ room, me, conn, hub }: GameViewProps) {
  ...
  useEffect(() => hub.subscribe((d0, at) => {
    ...
    switch (d.a) {
      ...
      case "sp_off":
        ...
        return;
      case "sp_hit":
        if (d.pod === me || (d.id && lightRef.current?.id === d.id)) { setFb({ txt: d.txt ?? (d.ms ? `${(d.ms / 1000).toFixed(2)}` : "✓"), good: d.good }); setTimeout(() => setFb(null), 1800); }
        if (isCoach) Sfx.pop();
        return;
      case "sp_miss":
        if (d.pod === me) { setFb({ txt: "פספוס", good: false }); setTimeout(() => setFb(null), 1500); Sfx.sadTrombone(); }
        return;
      case "sp_go":
        Sfx.goBeep(); Sfx.goBeep(120); vibrate(120);
        return;
      case "sp_say":
        if (isCoach) { setToast(d.t); setTimeout(() => setToast(""), 2600); }
  ...
  if (!s) return <main className="fullscreen sp-pod"><div className="pulse" style={{ fontSize: 60 }}>{def.icon}</div><p className="sub">מתחבר…</p></main>;
    ...
    if (light.zones) {
      return (
        <main className="sp-pod sp-lit" style={{ background: "#FFF3DC" }}>
          ...
          <div className="sp-zones-title">{light.txt ?? "גנוב!"}</div>
        </main>
      );
    }
    return (
      <main className="sp-pod sp-lit" style={{ background: c.hex, color: c.ink, filter: `brightness(${(0.08 + 0.92 * bright).toFixed(3)})` }} onPointerDown={(e) => { e.preventDefault(); tap(); }}>
        ...
        <div className="sp-txt">{light.txt ?? "גע!"}</div>
        {light.sub && <div className="sp-sub">{light.sub}</div>}
        {!light.ic && !light.fade && <div className="sp-hand">👆</div>}
        {light.home && <div className="sp-sub">גע בבית</div>}
      </main>
    );
  }
  ...
  return (
    <main className="sp-pod" style={{ background: flash ? "#FFFFFF" : "#07060A", border: col ? `10px solid ${col.hex}` : "10px solid #1C1A22" }}>
      ...
      <div className="sp-podlbl">פוד</div>
      {myAth && col && <div className="sp-chip" style={{ background: col.hex, color: col.ink }}>{nameOf(me)} · {col.name}{myAth.out ? " · 💀" : ""}</div>}
      {!myAth && <div className="sp-chip dim">{nameOf(me)} · פוד בלבד</div>}
      <div className="sp-banner">{s.banner}{s.phase === "count" && secsLeft > 0 ? ` · ${secsLeft}` : ""}</div>
      {s.phase === "setup" && <div className="sp-hint">הניחו אותי במקום, המסך למעלה 📱</div>}
      {s.phase === "over" && <div className="sp-hint">🏁 המשחק נגמר</div>}
      {myAth && s.phase !== "setup" && <div className="sp-score">{def.scoreLabel}: <b>{spFmtScore(game, myAth.score)}</b>{myAth.extra ? ` · ${myAth.extra}` : ""}</div>}
    </main>
  );
}
  ...
  return (
    <main className="sp-coach">
      ...
      <div className="sp-head">
        <div className="sp-title"><span className="sp-ic-s">{def.icon}</span><b>{def.name}</b><span className="sub">🎮 השלט של המאמן</span></div>
      ...
      <div className={"sp-bannerbox " + s.phase}>
        <b>{s.banner || def.name}</b>
        {s.sub && <small>{s.sub}</small>}
        {s.level ? <span className="chip">רמה {s.level}</span> : null}
      </div>

      {/* כפתורי שליטה */}
      <div className="sp-ctl">
        {setup && <button className="btn gold" disabled={!canStart} onClick={() => { Sfx.ding(); vibrate(40); send({ a: "sp_ctl", op: "start" }); }}>
          {canStart ? "🚀 התחל!" : nAth < def.minAth ? `צריך לפחות ${def.minAth} ספורטאים` : "צריך לפחות פוד אחד"}
        </button>}
        {running && <button className="btn ghost" onClick={() => send({ a: "sp_ctl", op: "pause" })}>⏸️ הפסקה</button>}
        {s.phase === "pause" && <button className="btn gold" onClick={() => send({ a: "sp_ctl", op: "resume" })}>▶️ המשך</button>}
        {(running || s.phase === "pause") && <button className="btn ghost" onClick={() => send({ a: "sp_ctl", op: "skip" })}>⏭️ דלג</button>}
        {(running || s.phase === "pause") && <button className="btn ghost danger" onClick={() => send({ a: "sp_ctl", op: "stop" })}>⏹️ סיים עכשיו</button>}
      </div>

      {setup && (
        <div className="card sp-card">
          <b>📐 סידור</b>
          <p className="sub" style={{ marginTop: 4 }}>{def.setup}</p>
          <p className="sub" style={{ fontSize: 12 }}>🛡️ הטלפונים מונחים מסך למעלה, נוגעים ביד — לא דורכים ולא בועטים.</p>
          {def.settings.map((st) => (
            <div key={st.key} style={{ marginTop: 8 }}>
              <div className="sub" style={{ fontSize: 12.5 }}>{st.label}</div>
              <div className="opt-row">
                {st.values.map((v) => <button key={v.v} className={"opt" + (s.cfg[st.key] === v.v ? " sel" : "")} onClick={() => send({ a: "sp_cfg", key: st.key, v: v.v })}>{v.label}</button>)}
              </div>
            </div>
          ))}
        </div>
      ...
      {/* ספורטאים */}
      <div className="card sp-card">
        <b>🏃 ספורטאים {nAth ? `(${nAth})` : ""}</b>
        {!setup && <span className="sub" style={{ marginRight: 8, fontSize: 12 }}>{def.scoreLabel}</span>}
        {ranked.length === 0 && <p className="sub">אין ספורטאים — כל טלפון שמצטרף הוא פוד + ספורטאי. סמנו "פוד בלבד" למי שלא רץ.</p>}
        {ranked.map((a, i) => {
          const c = spColor(a.c);
            ...
            <div key={a.pid} className={"sp-row" + (a.out ? " out" : "") + (inFocus ? " focus" : "")}>
              ...
              {setup ? (
                <span className="sp-rowctl">
                  ...
                  <select className="sp-sel" value={a.hand} onChange={(e) => send({ a: "sp_hand", pid: a.pid, ms: Number(e.target.value) })} title="הנדיקפ">
                    {SP_HAND_STEPS.map((h) => <option key={h} value={h}>{h ? `+${h / 1000} שנ'` : "בלי הנדיקפ"}</option>)}
                  </select>
                  <button className="opt tiny" onClick={() => send({ a: "sp_role", pid: a.pid, role: "pod" })}>פוד בלבד</button>
                </span>
              ) : (
                <span className="sp-rowctl">
                  <span className="sp-extra">{a.extra ?? ""}{a.med ? ` · ${(a.med / 1000).toFixed(2)}s` : ""}</span>
      ...
      {/* פודים */}
      <div className="card sp-card">
        <b>📱 פודים ({s.pods.length})</b>
        <p className="sub" style={{ fontSize: 12, marginTop: 2 }}>{setup ? "המספר מוצג על כל טלפון. \"הבהב\" עוזר לזהות איזה טלפון הוא איזה פוד." : "מפת הפודים"}</p>
        {s.pods.map((pid, i) => {
          const a = s.aths.find((x) => x.pid === pid);
          return (
            <div key={pid} className="sp-row">
              <b className="sp-podn">{i + 1}</b>
              <span className="sp-name">{nameOf(pid)}{a ? "" : " · פוד בלבד"}</span>
              {setup && <span className="sp-rowctl">
                ...
                <button className="opt tiny" onClick={() => send({ a: "sp_test", pod: pid })}>💡 הבהב</button>
                {!a && <button className="opt tiny" onClick={() => send({ a: "sp_role", pid, role: "ath" })}>🏃 ספורטאי</button>}
              </span>}
            </div>
          );
        })}
        {s.pods.length === 0 && <p className="sub">אף טלפון לא מחובר כפוד — סרקו את ה-QR מהלובי.</p>}
      </div>

      <p className="sub" style={{ fontSize: 11.5, textAlign: "center", marginTop: 4 }}>
        המשחק מסתיים לבד; "✕ סיום משחק" למעלה מחזיר ללובי בלי טקס.
      </p>
    </main>
  );
}
...
function phaseLabel(p: SpState["phase"]) {
  return p === "setup" ? "הכנה" : p === "count" ? "3-2-1" : p === "run" ? "▶" : p === "pause" ? "⏸" : p === "between" ? "…" : "🏁";
}
