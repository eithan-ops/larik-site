...
/** שיא שנשבר בחבורה — "האצבע הכי מהירה בחבורה: 0.38". label: מפתח group.rec.<k> (שיאים ישנים — מחרוזת בעברית) */
...
export type GameClientMsg = ForeheadClientMsg | PodsClientMsg | BombsClientMsg
  ...
  | WhoMostClientMsg | ShowClientMsg | ImpostorClientMsg | UndercoverClientMsg | WallClientMsg | HofrimClientMsg | ThievesClientMsg | AbyssClientMsg;
export type GameServerMsg = ForeheadServerMsg | PodsServerMsg | BombsServerMsg
  ...
  | WhoMostServerMsg | ShowServerMsg | ImpostorServerMsg | UndercoverServerMsg | WallServerMsg | HofrimServerMsg | ThievesServerMsg | AbyssServerMsg;
...
export const CATALOG: GameMeta[] = [
  ...
  },
  {
    ...
    name: "הקומות",
    icon: "🏢",
    tagline: "כולם מטפסים באותו מגדל. כל דקה — עצירה, ושדרוג אחד מתוך שישה.",
    howTo: "כל אחד בוחר דמות, וכולם מטפסים באותו מגדל בטלפון שלהם: אגודל לצדדים כדי לרוץ, טאפ בכל מקום כדי לקפוץ — וככל שרצים מהר יותר, קופצים גבוה יותר. הקיר מחזיר אתכם בלי לאבד מהירות. קפיצות של 2+ קומות ברצף = קומבו, והקומבו שווה נקודות בריבוע. המגדל עולה ומי שנופל מתחת למסך מאבד חיים (5). כל דקה — עצירה של 15 שניות: כולם בוחרים שדרוג אחד מתוך שישה, ורואים מה כולם לקחו. פטיש, כדור שלג, בננה, קפיצה כפולה… מי שמנצח — הכי הרבה קומות וקומבו.",
    minPlayers: 1,
    maxPlayers: 8,
    configOptions: [
      { key: "minutes", label: "אורך", values: [{ v: "8", label: "8 דק' 🔥" }, { v: "5", label: "5 דק' ⚡" }] },
    ],
  },
  {
    id: "tanks",
    name: "התותחים",
    icon: "💥",
    tagline: "כולם יורים באותה שנייה. מי שנשאר עומד — מנצח.",
    howTo: "כל אחד הוא טנק קטן על הר אחד, בטלפון שלו. 12 שניות לכוון: מותחים את האגודל אחורה כמו רוגטקה — הכיוון הוא הזווית, המרחק הוא העוצמה, וקו מקווקו מראה לאן הפגז יעוף. שימו לב לרוח! ואז — סלבו: כל הפגזים ממריאים יחד בכל הטלפונים, ההר מתפורר, ומי שנפגע מאבד חיים. אחרי כל סלבו נפתח המוסך: 12 שניות לקנות שדרוגים מבנק של 100+ קלפים — MIRV, נפאלם, מקדחה, מגן, טלפורט, קללות על חברים, מבול על כולם… וכולם רואים מה כל אחד קנה. מי שמת לא מחכה: כל סיבוב הוא מפיל מטאור או מברך חבר משמיים. שני קרבות, הבילד עובר ביניהם. מי שמנצח — הכי הרבה הריגות, שרידות וניצחונות.",
    minPlayers: 1,
    maxPlayers: 8,
    configOptions: [
      { key: "battles", label: "קרבות", values: [{ v: "2", label: "שניים 🙂" }, { v: "1", label: "אחד ⚡" }, { v: "3", label: "שלושה 🔥" }] },
      { key: "aim", label: "זמן כיוון", values: [{ v: "12", label: "12 שנ'" }, { v: "8", label: "8 שנ' ⚡" }, { v: "18", label: "18 שנ' 🧒" }] },
    ],
  },
  {
    id: "metro",
    name: "מטרונובול",
    icon: "🎾",
    tagline: "כל טלפון הוא כדור קופץ. אחד קובע את הקצב — כולם מכוונים אליו לפי האוזן.",
    howTo: "כל טלפון הוא מטרונום שנראה כמו כדור קופץ — כל נחיתה היא הצליל. בכל סבב שחקן אחד קובע את הקצב (10 שניות עם כפתורים, 50 רמות) ובוחר רצפה — פרקט, קרמיקה, גומי, מים… כל רצפה נשמעת אחרת. ואז 30 שניות: כולם מקישים על המסך בקצב ששומעים, והכדור של כל אחד קופץ בקצב ההקשות שלו. אף אחד לא רואה את המספר — רק שומעים את הכדור של הקובע ומנסים לנחות איתו. מי שנעל על הקצב מהר ומדויק מקבל הכי הרבה נקודות, וכשכל הכדורים נוחתים יחד — כולם ביחד! 🎶 בונוס לכולם. סולו: הקצב מתחלף כל 30 שניות — תפסו אותו.",
    minPlayers: 1,
    maxPlayers: 8,
    configOptions: [
