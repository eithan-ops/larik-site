...
try {
  ...
  await ev("pointerdown", 1, 250, 600); await ev("pointermove", 1, 270, 600); await sleep(700); const walk = await dbg(pg);
  // 2. גרירה מלאה (60px) → ריצה
  await ev("pointermove", 1, 310, 600); await sleep(700); const run = await dbg(pg);
  console.log("walk dx", walk.dx, "dir", walk.dir, "| run dx", run.dx, "dir", run.dir);
  // 3. הנפה למעלה = קפיצה
  await ev("pointermove", 1, 310, 590); await sleep(16); await ev("pointermove", 1, 310, 560); await sleep(120); const flick = await dbg(pg);
  console.log("flick st", flick.st, "(1/2 = באוויר)");
  await sleep(1200);
  // 4. אגודל ימין מוחזק = קפיצה אוטומטית שוב ושוב
  await ev("pointerdown", 2, 60, 700); const f0 = (await dbg(pg)).floor; let air = 0; for (let i = 0; i < 20; i++) { await sleep(100); if ((await dbg(pg)).st !== 0) air++; }
  const f1 = (await dbg(pg)).floor; await ev("pointerup", 2, 60, 700);
  console.log("hold-jump: air samples", air, "/20, floor", f0, "→", f1, "jumpHold after up:", (await dbg(pg)).jh);
  await ev("pointerup", 1, 310, 560); await sleep(100); console.log("after release dir", (await dbg(pg)).dir, "err", (await dbg(pg)).err);
