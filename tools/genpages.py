...
GAMES = [
  ...
  dict(id="impostor", name="המתחזה", icon="🎭", players="3–15", minutes="15–30",
    ...
    related=["undercover","whomost","deathtouch"]),
  dict(id="undercover", name="המתחזה למתקדמים", icon="🥸", players="3–12", minutes="15–30",
    title="המתחזה למתקדמים — גם למתחזה יש מילה | משחק חברה לטלפון | LARIK",
    desc="הגרסה המתקדמת של המתחזה: גם המתחזה מקבל מילה, ולכן אף אחד — כולל הוא — לא יודע מי החריג. רמזים, הצבעה סודית וחשיפה משותפת. בלי אפליקציה, בחינם.",
    h1="המתחזה למתקדמים — אולי אתה המתחזה ולא ידעת",
    intro="במתחזה הרגיל אחד מכם יודע שהוא המתחזה. כאן — אף אחד לא. כולם מקבלים מילה אמיתית, אבל לאחד מכם היא קצת אחרת: לכולם \"מנגו\", לו \"בננה\". עכשיו כל אחד אומר רמז אחד בקול, ותוך כדי אתה מתחיל להקשיב אחרת: למה כולם מדברים על משהו שלא לגמרי מתאיר למילה שלי? הרגע שבו נופל לך האסימון — \"רגע… אני המתחזה\" — הוא כל המשחק.",
    steps=["פותחים חדר ב-larik.ai וכולם סורקים QR — בלי להוריד כלום.","כל אחד מציץ בטלפון ורואה מילה. המסך נראה זהה אצל כולם: אין שום רמז מי החריג.","סבב רמזים לפי סדר: הטלפון של בעל התור נדלק, והוא אומר מילה אחת בקול.","מתווכחים, ואז כל אחד מצביע בסתר בטלפון — וכל המסכים נחשפים יחד: שתי המילים, מי הצביע למי, ומי היה המתחזה."],
    tips=["רמז כללי מדי מסגיר אותך בדיוק כמו רמז מדויק מדי — במיוחד כשאתה עוד לא בטוח מי אתה.","המארח יכול לבחור מילים \"קרובות 🔥\" לחבורות מנוסות, ולהדליק \"הכרזה עצמית\" — שמאפשרת למי שהבין שהוא המתחזה להכריז ולנחש את מילת הרוב לפני שתופסים אותו.","הכי טוב מ-5 שחקנים ומעלה; מ-7 יש שני מתחזים עם אותה מילה, שלא יודעים אחד על השני."],
    related=["impostor","whomost","alias"]),
...
<p class="tag">{len(GAMES)} משחקים לחבורה שנמצאת באותו חדר: המארח פותח חדר, כולם סורקים QR מהטלפון — ותוך 30 שניות משחקים. בלי הורדות, בלי הרשמה, בעברית ובחינם. מ"מי הכי?" ועד טריוויה, ממשחקי רפלקסים ועד משחק הרוצח.</p>
{hub_css}
...
(OUT / "index.html").write_text(page("games", f"משחקי חברה לטלפון בלי אפליקציה — {len(GAMES)} משחקים בחינם | LARIK",
...
collection("friends-night",
    ...
    ["whomost","impostor","undercover","forehead","alias"])
